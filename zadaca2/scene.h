...
class Scene 
{
  ...
  public:
    Vec3f cast_ray(Ray &ray, int depth) const
    {
        Vec3f hit_point;
        Vec3f hit_normal;
        Object* hit_object;
        ...
        if (scene_intersect(ray, hit_point, hit_normal, hit_object))
        {
            ...
            ray.direction = ray.direction - hit_normal * (2 * (ray.direction * hit_normal));
            depth --;
            return hit_object->material.diffuse_colour * diff + Vec3f(1, 1, 1) * spec + cast_ray(ray, depth)*hit_object -> material.reflectivity_coef;
        }
        else
        {
