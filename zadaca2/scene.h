  ...
  public:
    Vec3f cast_ray(const Ray &ray, int depth) const
    {
        ...
        if (scene_intersect(ray, hit_point, hit_normal, hit_object))
        {
            ...
            Ray reflection_ray = Ray(hit_point + hit_normal*0.02, ray.direction - hit_normal * (2 * (ray.direction * hit_normal)));
            Ray opacity_ray = Ray(hit_point + hit_normal*0.02, ray.direction);
            depth --;
            return hit_object->material.diffuse_colour * diff + Vec3f(1, 1, 1) * spec + cast_ray(reflection_ray, depth)*hit_object -> material.reflectivity_coef + cast_ray(opacity_ray, depth)*hit_object->material.opacity;
        }
        else
        {
